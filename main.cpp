    ...
    switch(key) {
        ...
        case 'P':
            ...
            if (strlen(config->stun_server) == 0) {
                ...
                if (stun_get_public_ip_imp(config->stun_server, config->stun_server_port, config->stun_local_port, ip) == 0)
                    fprintf(stderr, "Your public ipv4 address is: %s\n", ip);
                else
                    fprintf(stderr, "Get public ip failed, use `-v` before `-P` to get debug info.\n");
            }
...
const char *args_doc = "[-ehLopPqStTvxXV?]";
struct argp_option program_options[] = {
    ...
    {"ttl", 't', "ttl", 0, "Time-To-Live of the IP pachet, should be 1-255, default value is 64."},
    {"host", 'h', "host", 0, "Remote host to start a SYN FLOOD."},
    ...
    {"stun-server", 'S', "server", 0, "STUN server url or ipv4 address. This is used to get your public ipv4 address."},
    {"public-ip", 'P', 0, 0, "This is used to get your public ipv4 adress, if stun server is not set, dig will be used. If your want to use STUN, `stun-server`, `stun-server-port`, `stun-local-port` should be specified befor `-P`."},
